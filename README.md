...
- The same customer cannot have more than one active rental at the same time.
...
- Vehicles
  - `GET /api/vehicles?includeDeleted=false` (list)
  - `GET /api/vehicles?onlyOverFiveYears=true` (list only vehicles older than 5 years)
  - `GET /api/vehicles/over-five-years` (dedicated endpoint for vehicles older than 5 years)
  - `GET /api/vehicles/{vehicleId}` (by id)
  - `POST /api/vehicles` (create)
  - `PUT /api/vehicles/{vehicleId}` (update)
  - `DELETE /api/vehicles/{vehicleId}` (soft delete)
  - `GET /api/vehicles/available` (business projection)
- Rentals
  - `GET /api/rentals` (list)
  - `GET /api/rentals/{rentalId}` (by id)
  - `POST /api/rentals` (create reservation/rental)
...
1. Start PostgreSQL locally and ensure this connection string is valid in `src/GtMotive.Estimate.Microservice.Host/appsettings.Development.json`:
   - `Host=localhost;Port=55433;Database=renting;Username=postgres;Password=postgres`
...
- PostgreSQL: `localhost:55433` (`postgres/postgres`, database `renting`)
- Keycloak: `http://localhost:8081` (`admin/admin`)

### Authentication (professional local setup)

The docker-compose now includes Keycloak with a preloaded realm:

- Realm: `renting`
- Test user: `demo` / `demo123`
- Swagger OAuth client: `client-gtestimate-swagger` (public client, Authorization Code + PKCE)

How to authenticate from Swagger:

1. Open `http://localhost:8080/swagger`.
2. Click `Authorize`.
3. Log in with `demo` / `demo123`.
4. Execute protected endpoints with the generated bearer token.
...
- Docker profile in `src/GtMotive.Estimate.Microservice.Host/Properties/launchSettings.json`.
- Multi-stage Dockerfile in `src/GtMotive.Estimate.Microservice.Host/Dockerfile`.
- Root `.dockerignore` to reduce image build context.

This allows selecting Docker as startup profile directly from Visual Studio.
